...
struct dataset_data
{
  ...
  virtual void write_to_file_parallel(H5::Group &group, const std::string &dataset_name) const
  {
...
template <typename VT>
struct dataset_wattr : public dataset_attributes, dataset_data<VT>
{
  void print() const
  {
  ...
  void read_from_file_1proc(const H5::Group &grp, const std::string &dataset_name) override
  {
    ...
    // dataset<VT>::read_from_file_1proc(ds);
    dataset_attributes::read_from_file(ds);
  }

  void write_to_file_parallel(H5::Group &group, const std::string &dataset_name) const override
  {
    // dataset_data<VT>::write_to_file_parallel(group, dataset_name);
    auto ds = group.openDataSet(dataset_name);
    dataset_attributes::write_to_file_parallel(ds);
  }
};
